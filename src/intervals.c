...
int inter_cmp_by_start(const void* first, const void* second) {
    return ((indexed_weighted_interval*)first)->inter.start - ((indexed_weighted_interval*)second)->inter.start;
}

int inter_cmp_by_end(const void* first, const void* second) {
    return ((indexed_interval*)first)->inter.end - ((indexed_interval*)second)->inter.end;
}
...
int* schedule_intervals(alg_interval* intervals, size_t nitems) {
    indexed_interval* i_inters = malloc(sizeof(alg_interval) * nitems);
